...
async def init_database(settings: Settings | None = None) -> None:
    ...
    async with aiosqlite.connect(settings.database_path) as conn:
        ...
        await _ensure_column(conn, "users", "totp_enabled", "INTEGER NOT NULL DEFAULT 0")
        await _ensure_column(conn, "users", "totp_secret_enc", "TEXT")
        await _ensure_column(conn, "users", "totp_confirmed_at", "TEXT")
        await _ensure_column(conn, "users", "recovery_codes_hash", "TEXT")

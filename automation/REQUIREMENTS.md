...
- `health_check.py`：API连接、磁盘空间、文件权限等健康检查
- 三种运行模式赋予导演级干预能力
...
1. ~~SyntaxWarning: invalid escape sequence 第148行~~ **已修复**
...
| BLOG_OUTPUT_DIR | Hugo 输出目录 | /var/www/shetop.ru |
...
- **定时任务权限**：systemd service 使用 root 用户
- **网络访问**：仅在必要时开放 API 调用
